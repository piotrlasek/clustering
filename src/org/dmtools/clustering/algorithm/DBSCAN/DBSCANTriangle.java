...
public class DBSCANTriangle extends DBSCANBase {
    ...
    protected void TI_DBSCAN() {
        ...
        // set all points as CDMCluster.UNCLASSIFIED
        for (Point point : D) {
            ...
            while (seeds.size() > 0) {
                ...
                if (curPoint.NeighborsNo < MinPts) {
                    ...
                    while (curSeeds.size() > 0) {
                        ...
                        // q.NeighborsNo = q.NeighborsNo + 1;
                        q.NeighborsNo = q.NeighborsNo + 1;
                        // if q.ClusterId = CDMCluster.UNCLASSIFIED then
                        if (q.ClusterId == CDMCluster.UNCLASSIFIED) {
                            // q.ClusterId = ClId;
                            q.ClusterId = ClId;
    ...
    public void createSortedTableD(Collection<IClusteringObject> input,
        ...
        for (IClusteringObject o : input) {
            ...
            Point point = new Point(o.getSpatialObject().getValues());
    ...
    public void setData(IClusteringData data) {
        ...
        nDim = input.iterator().next().getSpatialObject().getValues().length;
    ...
    protected void determineBorderCoordinates() {
        minCoordinates = input.iterator().next().getSpatialObject().getValues().clone();
