...
public class MyFrame extends JPanel implements MouseListener, MouseWheelListener, MouseMotionListener
{
	...
	private void drawConstraints(Graphics g, Graphics2D g2) {
		...
		for(int i = 0; i < ic.ml1.size(); i++) {
			...
			CNBCRTreePoint p0 = ic.ml1.get(i);
			CNBCRTreePoint p1 = ic.ml2.get(i);
			g2.drawRect(getZoomed(p0.getCoord(0))-2*defMargin, getZoomed(p0.getCoord(1))-2*defMargin, pWidth + 4*defMargin, pWidth + 4*defMargin);
			g2.drawRect(getZoomed(p1.getCoord(0))-2*defMargin, getZoomed(p1.getCoord(1))-2*defMargin, pWidth + 4*defMargin, pWidth + 4*defMargin);
			g2.drawLine(getZoomed(p0.getCoord(0)), getZoomed(p0.getCoord(1)), getZoomed(p1.getCoord(0)), getZoomed(p1.getCoord(1)));
			
			
			
			g.setColor(Color.WHITE);
			g2.fillRect(getZoomed(p0.getCoord(0))-2*defMargin+1, getZoomed(p0.getCoord(1))-2*defMargin+1, pWidth + 4*defMargin-2, pWidth + 4*defMargin-2);
			g2.fillRect(getZoomed(p1.getCoord(0))-2*defMargin+1, getZoomed(p1.getCoord(1))-2*defMargin+1, pWidth + 4*defMargin-2, pWidth + 4*defMargin-2);
		...
		for(int i = 0; i < ic.cl1.size(); i++) {
			CNBCRTreePoint p0 = ic.cl1.get(i);
			CNBCRTreePoint p1 = ic.cl2.get(i);
			g2.drawRect(getZoomed(p0.getCoord(0))-2*defMargin, getZoomed(p0.getCoord(1))-2*defMargin, pWidth + 4*defMargin, pWidth + 4*defMargin);
			g2.drawRect(getZoomed(p1.getCoord(0))-2*defMargin, getZoomed(p1.getCoord(1))-2*defMargin, pWidth + 4*defMargin, pWidth + 4*defMargin);
		}
