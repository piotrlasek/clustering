...
public class CDNBCRTree implements IClusteringAlgorithm {
    ...
    ArrayList<CNBCRTreePoint> deferred = new ArrayList<CNBCRTreePoint>();
    ...
    public void run() {
        ...
        ArrayList<CNBCRTreePoint> def = new ArrayList<CNBCRTreePoint>();
        def.addAll(ic.cl1);
        def.addAll(ic.cl2);
        ...
        while (li.hasNext()) {
            ...
            if (p.clst_no != CNBCRTree.UNCLASSIFIED || p.clst_no == CNBCRTree.DEFERRED || p.ndf < 1)
                continue;
            ...
            if (existsCannotLink){
	            ...
	            for (int i = 0; i < kNN.neighbours.size(); i++) {
	                ...
	                if (q.clst_no != CNBCRTree.DEFERRED) {
		                q.clst_no = cluster_count;
		                if (q.ndf >= 1) {
		                    DPSet.add(q);
	
		                    // MUST-LINK
		                    ArrayList<CNBCRTreePoint> mls = ic.getMustLinkObjects(q);
		                    if (mls.size() > 0) {
		                    	DPSet.addAll(mls);
		                    }
		                }
        ...
        ArrayList<CNBCRTreePoint> cl = new ArrayList<CNBCRTreePoint>();
        cl.addAll(ic.cl1);
        cl.addAll(ic.cl2);
        ...
        while (li.hasNext()) {
            ...
            CNBCRTreePoint p = (CNBCRTreePoint) li.next();
            if (p.clst_no == CNBCRTree.UNCLASSIFIED)
                p.clst_no = CNBCRTree.NOISE;
        }
	...
	private void markDeferred(ArrayList<CNBCRTreePoint> def) {
		for(CNBCRTreePoint p : def) {
	        MyVisitor kNN = new MyVisitor();
	        tree.nearestNeighborQuery(k, p, kNN);
	        
            p.clst_no = CNBCRTree.DEFERRED;
            p.parentCL = p;
            //p.wasDeferred = true;
            deferred.add(p);
	        ...
	        for (int i = 0; i < kNN.neighbours.size(); i++) {
	        	CNBCRTreePoint pcl1 = (CNBCRTreePoint) kNN.neighbours.get(i);
	            pcl1.clst_no = CNBCRTree.DEFERRED;
	            pcl1.parentCL = p;
    ...
    private void recluster2(ArrayList<CNBCRTreePoint> in) {
    	ArrayList<CNBCRTreePoint> toRemove = new ArrayList<CNBCRTreePoint>();
        int step= 0;
        do {
        	toRemove.clear();
        	
        	for(CNBCRTreePoint p : in) {
        		
        		ArrayList<CNBCRTreePoint> neighbors = getNeighbors(p, 2*k);
        		
        		for(CNBCRTreePoint q : neighbors) {
        			if (q.ndf >= 1 && q.clst_no >= 0  && !q.wasDeferred) {
        				
        				boolean cl1 = false;
        				
						 if (ic.cannotAsignToGroup(p, q.clst_no)) {
							 cl1 = true;
						 }
						 
						 if (ic.cannotAsignToGroup(getNearestCL(p, k), q.clst_no)) {
							 cl1 = true;
						 }
...
//	        				}
	...
	private void recluster() {
        ...
        do {
        	...
        	for(CNBCRTreePoint p : deferred) {
        		
                ArrayList<CNBCRTreePoint> neighbors = getNeighbors(p, k);
                ...
                for (CNBCRTreePoint q  : neighbors) {
                	 if (q.ndf >=1 && q.clst_no >= 0 && !q.wasDeferred ) {
                		 ...
                		 */
                		 ...
                		 if (ic.cannotAsignToGroup(p.parentCL, q.clst_no)) {
                			 cl1 = true;
                		 }
                		 
                		 if (ic.cannotLink(p.parentCL, q.parentCL)) {
                			 cl1 = true;
                		 }
        ...
        for (CNBCRTreePoint tp : deferred) {
        	
        	
        	ArrayList<CNBCRTreePoint> nbs = getNeighbors(tp, k);
        	
        	for (CNBCRTreePoint n : nbs) {
        		if (!n.wasDeferred && n.clst_no >=0) {
	...
	private void setConstraints() {
		CNBCRTreePoint p0 = new CNBCRTreePoint(new double[]{438.0, 259.0}, CNBCRTree.UNCLASSIFIED);
    	CNBCRTreePoint p1 = new CNBCRTreePoint(new double[]{440.0, 255.0}, CNBCRTree.UNCLASSIFIED);    	
    	CNBCRTreePoint p2, p3, p4; 
    	
    	p0 = new CNBCRTreePoint(new double[]{443.0, 271.0}, CNBCRTree.UNCLASSIFIED); 
    	p0 = Dataset.get(2730);
    	p1 = new CNBCRTreePoint(new double[]{448.0, 278.0}, CNBCRTree.UNCLASSIFIED);
    	p2 = new CNBCRTreePoint(new double[]{421.0,333.0}, CNBCRTree.UNCLASSIFIED);
    	p3 = new CNBCRTreePoint(new double[]{433.0,325.0}, CNBCRTree.UNCLASSIFIED);
    	...
    	p0 = new CNBCRTreePoint(new double[]{457.0, 334.0}, CNBCRTree.UNCLASSIFIED);
    	p1 = new CNBCRTreePoint(new double[]{467.0, 340.0}, CNBCRTree.UNCLASSIFIED);
    	p2 = new CNBCRTreePoint(new double[]{475.0, 348.0}, CNBCRTree.UNCLASSIFIED);
    	p3 = new CNBCRTreePoint(new double[]{478.0, 356.0}, CNBCRTree.UNCLASSIFIED);
    	p4 = new CNBCRTreePoint(new double[]{508.0, 373.0}, CNBCRTree.UNCLASSIFIED);
    	...
    	p0 = new CNBCRTreePoint(new double[]{326.0, 314.0}, CNBCRTree.UNCLASSIFIED);
    	p1 = new CNBCRTreePoint(new double[]{359.0, 369.0}, CNBCRTree.UNCLASSIFIED);
    	p0 = Dataset.get(Dataset.indexOf(p0));
    	p1 = Dataset.get(Dataset.indexOf(p1));
    	ic.addMustLinkPoints(p0, p1);
	}
